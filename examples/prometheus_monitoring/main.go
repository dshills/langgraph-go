...
func printMetricsSummary(runCount int) {
	...
	log.Println("  - langgraph_backpressure_events_total: Queue saturation events")
	log.Println("")
	...
	log.Println("  - rate(langgraph_backpressure_events_total[5m])")
	log.Println("")
	log.Println("BACKPRESSURE METRICS (T033):")
	log.Println("  Backpressure occurs when the scheduler queue reaches capacity.")
	log.Println("  This example uses QueueDepth=64 and MaxConcurrent=8.")
	log.Println("  When queue fills up (depth >= 64), backpressure_events_total increments")
	log.Println("  with label reason='queue_full'.")
	log.Println("")
	log.Println("  Example output from metrics endpoint:")
	log.Println("    langgraph_backpressure_events_total{reason=\"queue_full\",run_id=\"run-1\"} 3")
	log.Println("    langgraph_backpressure_events_total{reason=\"queue_full\",run_id=\"run-2\"} 5")
	log.Println("")
	log.Println("  To trigger backpressure:")
	log.Println("    - Run with slow nodes and high parallelism (many branches)")
	log.Println("    - Monitor queue_depth: when it approaches 64, backpressure metrics trigger")
	log.Println("    - Higher concurrency + slower nodes = more backpressure events")
	log.Println("===============================================")
	log.Println("")
}
